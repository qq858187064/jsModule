<%@ Page Title="表单批处理组件" Language="C#" CodeFile="Forms20141105 .aspx.cs" Inherits="Forms" %>
...
<label class="Rmb"><a class="BtnLg"  href="javascript:var F=C.G('Cm1s');Forms.Vf(F);alert(F.D);">登&nbsp;录</a><label class="Ilb" for="RmbPwdn"><input id="RmbPwdn" type="checkbox" title="1年内保持登录"  />保持登录</label></label>
</form>
...
<label for="Pd"><b>密码：</b><input id="Pd" name="" type="text" Ps='Preset:"Rqd Cn",Regex:"/^\S{2,10}$/",Cem:"该字段长度为2~10个字符"' Preset="Rqd" /><i>*</i></label>
...
<a href="####" onclick="Forms.Vf('Sm2');alert('该表单验证结果：'+C.G('Sm2').V);" class="BtnLg">注&nbsp;册</a>
    ...
    <div id="Cc1s" class="AjLg"><%-- Preset="Rqd Cn" Regex="/^\S{2,10}$/" Cem="姓名长度须在2-10之间" --%>
<label for="Nma">用户名：<input type="text" id="Nma" class="Txt" p="Preset:'Rqd Cn',Regex:'/^\S{2,10}$/',Cem:'姓名长度须在2-10之间'" onclick="alert(C.Pt(C.G('Nma')).Regex)"/><i id="NmRta"></i></label>
<label for="Pwda">密&nbsp;&nbsp;&nbsp;码：<input type="password" id="Pwda" class="Txt" Preset="Rqd Pwd" Regex="/^\S{6,128}$/" Cem="密码长度须在6-128位之间"  /><i id="PwdRta"></i><a href="#">忘记密码？</a></label>
<label class="Rmb"><a class="BtnLg"  href="javascript:var F=C.G('Cc1s');Forms.Vf(F);alert(F.D);">登&nbsp;录</a><label class="Ilb" for="aRmbPwdn"><input id="aRmbPwdn" type="checkbox" title="1年内保持登录"  />保持登录</label></label>
</div>
...
<p>接着验证Regex属性所定的的正则表达式，指定该属性的元素也需要指定Cem属性，用来指定自定义错误信息，验证结果默认显示在验证元素后一个兄弟元素中，如果不希望这样，则为元素定义Rpt属性，用来指定显示验证结果元素的ID，显示验证结果的元素需要预置两个Css样式Ok、No，分别用来定义验证成功和验证失败时的样式。</p><p>如果需要自行对ID为"Sm2s"的表单进行验证，则可以var Fm=C.G("Sm2s");Fm.Vf();调用Vf方法之后，便可以用Fm.V属性获取表单验证的结果了,其值为Boolean类型，也可以获取Fm.D属性，其值为表单中元素的键值对，radio的键为name，如果不想取某元素的值，应将其id以n后缀。</p>
<h6>该模块调用代码:</h6>
...
<asp:Content ID="ContentRunJs" ContentPlaceHolderID="RunJs" Runat="Server">window.Cm=14;Forms("Sm2");
</asp:Content>